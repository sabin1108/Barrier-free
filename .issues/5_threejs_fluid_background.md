...
- [ ] 메인 및 로그인 화면 배경에 동적인 3D 유체 시뮬레이션(Three.js/R3F) 효과가 적용되어야 함
- [ ] 3D 렌더링 루프 실행 시에도 메인 스레드 점유율이 30% 이하로 유지되어 성능 저하가 없어야 함
- [ ] 초당 프레임(FPS) 수가 60 이상으로 부드럽게 렌더링되는 성능 기준을 달성해야 함
